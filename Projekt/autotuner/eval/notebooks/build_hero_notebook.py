...
c.append(nbf.v4.new_code_cell(
    ...
    "FLOPS = 2 * N**3\n"
    "\n"
    "# --- Ergebnis-Speicher: EINMAL messen -> hero_results.json -> Charts laden nur ---\n"
    "# REMEASURE=True erzwingt Neumessung (misst FRISCH, TVM ~2-10 min) und ueberschreibt\n"
    "# die JSON. REMEASURE=False (Standard) LAEDT die gespeicherten Ergebnisse -> Charts/\n"
    "# Text aendern (z.B. Umlaute) kostet dann KEINE Neurechnung.\n"
    "REMEASURE = False\n"
    "TORCH_EINSUM = 2301.04  # Nachmessung 2026-08-13, results/torch_einsum_hero.json: einsum ~ matmul-Niveau\n"
    "\n"
    "DIR = os.getcwd()\n"
    ...
    "\n"
    "import json\n"
    "RESULTS_JSON = os.path.join(DIR, 'hero_results.json')\n"
    ...
    "        loaded = json.load(f)\n"
    "    results = {k: loaded[k] for k in ORDER}\n"
...
c.append(nbf.v4.new_code_cell(
    ...
    "ax.set_yscale('log')\n"
    "ax.set_ylabel('GFLOPS  (log-Skala)', fontsize=14)\n"
    "ax.set_title(f'Technologie-Vergleich: {CASE} @ {N}\\u00b3  \\u2014  Apple M4 Max', fontsize=15, fontweight='bold')\n"
    "ax.tick_params(axis='y', labelsize=13)\n"
    "ax.tick_params(axis='x', labelsize=10.5)\n"
    "ax.set_ylim(1, max(vals) * 2.4)\n"
    ...
    "                ha='center', color='#444', fontsize=12, fontweight='bold')\n"
    "plt.tight_layout()\n"
...
c.append(nbf.v4.new_markdown_cell(
    ...
    "- **Bonus:** `torch.einsum` (statt `torch.matmul`) erreicht für denselben GEMM praktisch "
    "dasselbe Niveau (**~2301 vs. ~2568 GFLOPS**, Nachmessung in `results/torch_einsum_hero.json`) — "
    "PyTorch erkennt das GEMM-Muster und nutzt denselben BLAS-Pfad. Der API-Pfad ist hier also *kein* "
    "relevanter Faktor.\n"
    "\n"
