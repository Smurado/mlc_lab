...
# REMEASURE=True misst FRISCH auf IDLE-Maschine (Standard). ACHTUNG: TVM-Tuning dauert
# Minuten pro Groesse (genau das ist der Punkt!). Auf gecachte Werte: REMEASURE = False.
REMEASURE = True

CACHED = __CACHED__  # {N: (teir_s, teir_gflops, tvm_s, tvm_gflops)}
...
for N in SIZES:
    if REMEASURE:
        ...
        tt, tg, vt, vg = CACHED.get(N, (float('nan'),) * 4)
...
a1.set_yscale('log'); a1.set_xticks(x); a1.set_xticklabels([f'{n}³' for n in N])
a1.set_ylabel('Tuning-Zeit [s]  (log)'); a1.set_title('Tuning-KOSTEN (kleiner = besser)')
for i, r in enumerate(rows):
    a1.text(i - w/2, r[1]*1.1, f'{r[1]:.0f}s', ha='center', va='bottom', fontsize=9, fontweight='bold')
    a1.text(i + w/2, r[3]*1.1, f'{r[3]:.0f}s', ha='center', va='bottom', fontsize=9, fontweight='bold')
a1.legend(); a1.grid(axis='y', alpha=0.3, which='both')
...
a2.set_xticks(x); a2.set_xticklabels([f'{n}³' for n in N])
a2.set_ylabel('Kernel-Leistung [GFLOPS]'); a2.set_title('Kernel-QUALITAET (groesser = besser)')
for i, r in enumerate(rows):
    a2.text(i - w/2, r[2]*1.02, f'{r[2]:.0f}', ha='center', va='bottom', fontsize=9, fontweight='bold')
    a2.text(i + w/2, r[4]*1.02, f'{r[4]:.0f}', ha='center', va='bottom', fontsize=9, fontweight='bold')
a2.legend(); a2.grid(axis='y', alpha=0.3)

fig.suptitle('Trade-off: Tuning-Zeit vs. Kernel-Qualitaet (GEMM, Apple M4 Max)', fontweight='bold')
plt.tight_layout(); plt.savefig('tuningtime_comparison.png', dpi=140); plt.show()
'''
